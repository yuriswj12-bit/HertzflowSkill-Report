...
| 维度 | 结论 | 关键证据 |
|---|---|---|
| **当前阶段** | 🔴 MM/机器人主导的分发交易 | MM 候选 170 个; 可见卖出下界 $5.45M |
| **筹码结构** | 🟣 高控盘 | Top10 持仓 63.44%; Top1 持仓 58.66% |
| **资金/分发结构** | 🔴 多集群同源分发 | 最大分发源 `E99pfU...HUTe` 连接 78 个下游 |
| **做市/MM 地址集群** | 🔴 已显著浮现 | bundler 钱包 1000 个; 前 5 分发集群 MM 命中 78 个 |
| **派发/套现情况** | 🟠 样本内部分套现 | GMGN top traders 卖出下界 $5.45M; 买入额 $7.26M |
| **盘口阶段** | 🟠 中市值 + 薄承接 | mcap $135.68M; LP $1.42M; 24h volume $52.86M |
...
- **当前行情**: 价格 `$0.13568241`，市值约 **$135.68M**，DEX 主池流动性约 **$1.42M**，holders **34,649**。
- **核心风险**: Top10 持仓 **63.44%**，Top1 `GV6UUm...dC52` 单独持仓 **58.66%**。
- **MM / 做市集群**: 识别到 **170** 个 MM/机器人候选地址；最大分发源 `E99pfU...HUTe` 连接 **78** 个下游，其中 **61** 个命中 MM/机器人特征。
- **已确认样本内卖出下界**: top traders 可见卖出约 **$5.45M**。注意这只是 GMGN 返回样本下界，不等于全链真实出货总额。
...
_工具版本 sol_meme forensic view · 主链 Solana · 生成时间 2026-06-30 02:24:58 UTC_
...
| 主战场链 | Solana |
| **当前价** | **$0.13568241** |
| **24H 成交** | **$52.86M** (buy $26.44M / sell $26.42M, swaps 190,906) |
| 当前 LP (DEX 主池) | $1.42M |
| 市值 / 供应 | $135.68M / 999.99M |
| 数据来源 | GMGN CLI · 持币 34,649 个 · Smart/KOL 78/41 |
...
| 主战场链 | solana |
| **进场上限提示** | 主池 LP $1.42M，但 Top1/Top10 控盘过高，不能用 LP 单独判断承接 |
...
**GMGN 样本内识别到 3 个综合关系集群、13 个分发来源集群、23 个同源资金集群、2 个收集/回流去向集群。**

| 决策锚点 | 数值 | 状态 |
|---|---:|---|
| DEX 主池 USD 流动性 | $1.42M | 🟠 |
| Top10 holder rate | 63.44% | 🔴 高控盘 |
| GMGN bundler wallets | 1000 | 🔴 机器人/打包痕迹重 |
| 样本内卖出下界 | $5.45M | 🟠 部分套现已发生 |

## 🧠 当前链上行为画像

| 严重度 | 标签 | 类别 | 触发指标 | 链上事实 |
|:-:|---|---|---|---|
| 🔴 **STRONG** | `S-C1` 同源分发集群 | C 出货行为 | `distribution_clusters=13` | 多个地址从同一上游收币后卖出或继续转出，最大分发源连接 78 个下游 |
| 🔴 **STRONG** | `S-R1` 综合资金关系集群 | D 协同结构 | `relationship_clusters=3` | 同资金来源、同分发源、同回流地址、相近 funding 时间/金额、同 bot 栈被合并成地址集群 |
| 🔴 **STRONG** | `S-B1` MM/机器人做市 | B 成交制造 | `mm_candidates=170, bundlers=1000` | 大量 bundler/dex_bot/axiom/photon/padre 地址，高成交低库存，符合做市/刷量/分发下游特征 |
| 🔴 **STRONG** | `S-A1` 高控盘 | A 筹码结构 | `top10=63.44%, top1=58.66%` | Top1/Top10 持仓过高，外部可验证散户抛压难以充分确认 |
| 🟠 **MEDIUM** | `S-C2` 样本内套现 | C 出货行为 | `sell_lower=$5.45M` | top traders 可见卖出为真实链上成交样本下界，不含 CEX/OTC/跨钱包后续兑换 |
...
> **速读**: 总供应约 999.99M · **操盘/MM/机器人相关 14.79%** (前 5 分发集群当前持仓合计) · DEX 主池流动性 $1.42M · 样本内可见卖出下界 $5.45M。

| 筹码桶 | 估算 | 解读 |
|---|---:|---|
| 🟣 **操盘/MM/机器人相关** | **14.79%** | 前 5 个分发集群当前持仓合计；可见卖出 $6.16M |
| 🟦 **DEX 池 / 流动性侧** | **$1.42M** | 主池流动性，不等同于可承接真实卖压 |
| 🔥 **可验证非集群散户抛压** | **未充分确认** | SOL MVP 暂无完整 transfer graph，只能把非集群地址作为待排除对象 |

#### 🟣 前 5 个分发集群明细

> 上面的 **14.79%** 不是凭空估算，而是下表 5 个分发源当前持仓占比相加。卖出额是 GMGN 归一化后的 USD 估算；实际链上兑现资产是池子 quote 资产，这里按主池 SOL/USD ≈ $74.30 折算为 SOL。当前USD是未兑现库存估值，不计入“已兑现”。

| # | 分发源 | AI解读 | 下游地址数 | MM命中 | 当前持仓 | 已实现利润 | 未实现利润 | 未兑现库存USD | 卖出额USD | 已兑现SOL估算 | 代表下游 |
|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | `E99pfU...HUTe` | 已经盈利，主要利润仍在库存 | 78 | 61 | 9.93% | $2.18M | $8.53M | $13.43M | $3.30M | 44,415 SOL | 3CPHk5...FkWY, 9H9HU7...C4AZ, F5hkYs...xqs6, 3inTyf...rTg2, 8e9bkF...6qg6 |
| 2 | `GV6UUm...dC52` | 已经盈利，主要利润仍在库存 | 10 | 7 | 1.47% | $777,276 | $1.72M | $1.99M | $1.12M | 15,028 SOL | 5URyNU...c19Y, HDixbr...XN3o, 5FGoPP...rp8V, 6btcCb...g5eT, DAEdBm...CvHC |
| 3 | `Dtw3GC...p2hs` | 已经盈利，主要利润仍在库存 | 6 | 4 | 1.66% | $814,996 | $1.02M | $2.24M | $1.34M | 18,079 SOL | 3H9LVH...FSEC, HTpoBR...cS9y, HCgo8g...2uYC, 6Hcrxu...8UMg, GkdYWR...Atac |
| 4 | `B4Z8JS...JdpF` | 已经盈利，主要利润仍在库存 | 3 | 3 | 1.72% | $587,140 | $2.19M | $2.33M | $382,294 | 5,145 SOL | CLM6E4...Kg1Q, CxCTVj...rdtT, ACTbvb...y831 |
| 5 | `6jmUvt...6xz1` | 已经盈利，主要利润仍在库存 | 3 | 3 | 0.01% | $1,305 | $2,112 | $8,078 | $14,275 | 192.13 SOL | 6msXZd...VmGW, CEq8qb...rtz5, DrnuP4...kE67 |
...
| emoji | 类别 | 数量 | 解读 |
|---|---|---:|---|
| 🔴 | 同一分发来源集群 | 13 | 多地址从同一上游收币后卖出/转出，疑似操盘下游 |
| 🔴 | MM/机器人候选 | 170 | bundler/dex_bot/axiom/photon/padre + 高成交低库存 |
| 🟠 | 样本内套现地址 | 142 | 已出现高卖出额、高已卖比例或大额转出 |
...
- 第三段 套现/回流: top traders 可见卖出 $5.45M，且出现同一回流去向，说明不是单向自然换手。
...
- **主控集群已经成型**: `cluster_01` 覆盖 203 个样本钱包，MM/机器人命中 159 个，套现命中 126 个，当前持仓约 **75.45%**。
...
- **分发已经发生**: 样本内最大卖出成员 `3H9LVH...FSEC` 可见卖出 **$718,653**；主集群累计可见卖出 **$7.37M**。
...
| 观察点 | 解释 | 用户该怎么用 |
|---|---|---|
| 主集群持仓 75.45% | 控盘筹码仍然集中在同一关系图内 | 不要把 holder 数量当成分散度，优先看主集群是否继续转出 |
| 主集群已卖 $7.37M | 已出现边做市边兑现的行为 | 如果后续卖出继续增加，说明派发未结束 |
...
|---|---|---:|---:|---:|---|
| `cluster_01` | 主控/MM集群 | 203 | 75.45% | $7.37M | same_sol_funder:3a8DRp...gLVj / same_token_distributor:yHCxHB...6PRe / same_token_collector:5URyNU...c19Y |
...
| P0 | `E99pfUw985q5qk5Aa6ERXqJwsBjoE1igPwNfHBabHUTe` | 最大分发源 | 连接 78 个下游，MM命中 61 个 | 继续向新地址拨币 |
...
```json
{
  ...
  "token": {
    ...
    "symbol": "ANSEM",
    "name": "The Black Bull",
    "price_usd": 0.13568241,
    "market_cap_usd": 135680727.26675117,
    "liquidity_usd": 1422378.391770702,
    "holders": 34649
  },
  "verdict": {
    "risk_score": 10,
  ...
  "metrics": {
    "top10_holder_rate": 0.6344,
    "top1_holder_rate": 0.5866347304324788,
    "bundler_wallets": 1000,
    "mm_candidate_count": 170,
    "cashout_candidate_count": 142,
    "sell_lower_bound_usd": 5445018.526815146,
    "buy_sample_usd": 7255051.753923232,
    "distribution_cluster_count": 13,
    "funding_cluster_count": 23,
    "collection_cluster_count": 2,
    "relationship_cluster_count": 3
  },
  "top_clusters": {
    "relationship": [
      {
        ...
        "mm_count": 159,
        "cashout_count": 126,
        "hold_rate": 0.7544791633410988,
        "buy_usd": 8365547.233330381,
        "sell_usd": 7368995.876481623,
        "transfer_in_amount": 762867097.5364239,
        "transfer_out_amount": 832399534.479951,
        "features": [
        ...
        "representatives": [
          ...
          "HTpoBRnq1fv7AzDgWi6VULf2TCdGkseNdMqoCJJZcS9y",
      ...
      },
      {
        ...
        "mm_count": 0,
        "cashout_count": 1,
        "hold_rate": 0.010500136138131385,
        "buy_usd": 2029.92782975274,
        "sell_usd": 0.0,
        "transfer_in_amount": 10541814.658034,
      ...
      },
      {
        ...
        "mm_count": 0,
        "cashout_count": 0,
        "hold_rate": 0.0030000342064452285,
        "buy_usd": 0.0,
        "sell_usd": 0.0,
        "transfer_in_amount": 2999997.0000210004,
    ...
    "distribution": [
      {
        ...
        "wallet_count": 78,
        "mm_count": 61,
        "hold_rate": 0.09932157424808583,
        "buy_usd": 5475768.930826142,
        "sell_usd": 3300000.4130949806,
        "representatives": [
          "3CPHk5b7SRvAsTsocehAu1bA5YKMXJYMr3qc2SuMFkWY",
      ...
      },
      {
        ...
        "wallet_count": 10,
        "mm_count": 7,
        "hold_rate": 0.014706442538602364,
        "buy_usd": 110548.39861765967,
        "sell_usd": 1116574.4988178434,
        "representatives": [
          "5URyNUmhcuWdZiiQrtNdFrSbQPfq72UV2gqQasr9c19Y",
      ...
      },
      {
        ...
        "wallet_count": 6,
        "mm_count": 4,
        "hold_rate": 0.01657621401861326,
        "buy_usd": 1221848.8545071185,
        "sell_usd": 1343276.0573903285,
        "representatives": [
          "3H9LVHarjBoZ2YPEsgFbVD1zuERCGwfp4AeyHoHsFSEC",
          "HTpoBRnq1fv7AzDgWi6VULf2TCdGkseNdMqoCJJZcS9y",
      ...
      },
      {
        ...
        "wallet_count": 3,
        "mm_count": 3,
        "hold_rate": 0.01723507826697067,
        "buy_usd": 140550.39895216134,
        "sell_usd": 382293.62331823073,
        "representatives": [
      ...
      },
      {
        ...
        "wallet_count": 3,
        "mm_count": 3,
        "hold_rate": 5.960390087957871e-05,
        "buy_usd": 18843.19752484524,
    ...
    "funding": [
      {
        ...
        "wallet_count": 19,
        "mm_count": 19,
        "hold_rate": 0.006080184241555963,
        "buy_usd": 418912.2517499978,
        "sell_usd": 271551.02507921174,
        "representatives": [
      ...
      },
      {
        ...
        "wallet_count": 15,
        "mm_count": 10,
        "hold_rate": 0.009674528918891652,
        "buy_usd": 336946.5137139514,
        "sell_usd": 252870.97024627656,
        "representatives": [
      ...
      },
      {
        ...
        "wallet_count": 5,
        "mm_count": 5,
        "hold_rate": 0.012677130730337318,
      ...
      },
      {
        ...
        "wallet_count": 5,
        "mm_count": 5,
        "hold_rate": 0.00484455968472121,
        "buy_usd": 405940.5476338621,
      ...
      },
      {
        ...
        "wallet_count": 6,
        "mm_count": 4,
        "hold_rate": 0.008702253620575402,
        "buy_usd": 199889.66159328236,
        ...
        "representatives": [
          ...
          "3T58aoVUMAdfSJctit9TqZGXjUYucTbqFBjr2VuQU2vG"
        ]
      }
    ],
    "collection": [
      {
        ...
        "wallet_count": 2,
        "mm_count": 2,
        "hold_rate": 0.0008229217818089379,
        "buy_usd": 178543.0055755999,
      ...
      },
      {
        ...
        "wallet_count": 2,
        "mm_count": 1,
        "hold_rate": 0.0033499708528855173,
        "buy_usd": 507787.57943636534,
        "sell_usd": 586052.6613064841,
        "representatives": [
          "2S8E25nAcqvvMYEWZHBQk22GzevwvmsVNAs6Gw5tRX7d",
          "8e9bkF9q2cZqRpLqGNgimr19mhHarW7p1yvSreTC6qg6"
        ]
